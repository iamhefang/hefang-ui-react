...
import {execute, extend, guid, type} from "hefang-js";
...
export function Progress(props: ProgressProps) {
	const realProps: ProgressProps = extend(true, {}, defaultProps, props);
	const {
			value, max, strokeColor, size, text, id = guid(), showCursor,
			style: {
		...
		,
		progressBackground = Array.isArray(strokeColor) ? `linear-gradient(90deg, ${strokeColor.join(", ")})` : strokeColor
		, height = sizes[size]
